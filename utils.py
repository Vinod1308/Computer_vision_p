...
def dict_to_csv(output: dict, csv_file_path, attendance_date):
    roll_numbers = set()
    students = {}
    attendance_data = {}  # Collect all attendance data in a single dictionary

    for frame_time, frame_attendance in output.items():
        for student, status in frame_attendance.items():
            roll_number, name = separate_name_roll(student)
            roll_numbers.add(roll_number)
            students.setdefault(roll_number, {'Name': name})

            attendance_data.setdefault(student, {}).setdefault(attendance_date, {})[frame_time] = status  # Store attendance data

    # Write the data to CSV
    with open(csv_file_path, 'w') as f:
        w = csv.writer(f)
        header = ['Roll Number', 'Name', f"{attendance_date}_Attendance"]
        w.writerow(header)

        for roll_number in roll_numbers:
            student_data = students[roll_number]
            attendance_row = []

            for frame_time in output.keys():
                attendance_status = attendance_data.get(roll_number, {}).get(attendance_date, {}).get(frame_time, 'Absent')
                attendance_row.append(attendance_status)

            row = [roll_number, student_data['Name'], ', '.join(attendance_row)]  # Combine attendance data
            w.writerow(row)
...
def clean_duplicate_attendance(csv_file_path,attendance_date):
    ...
    with open(csv_file_path, 'r') as f:
        ...
        for row in reader:
            ...
            name = row['Name']
            attendance = row[f"{attendance_date}_Attendance"]
            unique_attendance = set(attendance.split(', '))  # Convert to set to remove duplicates
            cleaned_attendance = ', '.join(unique_attendance)  # Convert back to string

            cleaned_data.append({
                'Roll Number': roll_number,
                'Name': name,
                f"{attendance_date}_Attendance": cleaned_attendance
            })
